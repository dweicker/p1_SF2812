...
capacities(i,j,scenario).. x(i,j)+x(j,i)+y(i,j,scenario)+y(j,i,scenario) =l= maxX*(actualCap(i,j,scenario)+actualCap(j,i,scenario));
consB(i).. sum(j,x(i,j)) =e= sum(j,x(j,i)) + e(i);
consA(i,scenario).. sum(j,y(i,j,scenario)) =e= sum(j,y(j,i,scenario)) + f(i);

model part4 /all/;

solve part4 using lp minimizing maxX;
